...
var postTime = (postedTime) => {
  ...
  var monthFormat = dayjs(postedTime).format('DD MMM YYYY HH:mm')
  var dayFormat = dayjs(postedTime).format('ddd HH:mm')
      ...
      theTimeToShow=monthFormat
       ...
       if(weekDiff>=1){
        ...
        if(daysDiff>1){
         ...
         if(hourDiff>=1){
          if(hourDiff===1){theTimeToShow='an hour ago'}else{theTimeToShow=hourDiff+' hrs ago'}
         }else{
          if(minDiff>=1){
          if(minDiff===1){theTimeToShow='a min ago'}else{theTimeToShow=minDiff+' mins ago'}
          }else{
            if(secDiff<2){theTimeToShow='a sec ago'}
            else{theTimeToShow=secDiff+' sec ago'}
