...
var week1RoundEdit=[],week2RoundEdit=[],week3RoundEdit=[],superBowlEdit=[],compItems=[]

var week1RoundEdit2=[],week2RoundEdit2=[],week3RoundEdit2=[],superBowlEdit2=[]
class NCAAModal extends Component {
  state = { week1RoundEdit:[], week2RoundEdit:[], week3RoundEdit:[], superBowlEdit:[], submitErr: "", showProgressBar: false, currentSelection:this.props.eventToNFLModal,isItSubmit:false,
  ...
  componentDidMount=()=>{
   ...
   // return
    if(incomingData.length>0){
      week1RoundEdit=[],week2RoundEdit=[],week3RoundEdit=[]
      var firstMatchTime=[]
        var i=0
  ...
  conferenceChampionshipSubmit = () => {
    ...
    var yearNow = new Date().getFullYear()
    var k = 0, l = 0
    this.state.week3RoundEdit.map((item, index) => {
      if (item.time === '') {
        week3RoundEdit[index]['error'] = 'Match time field must be filled'
        this.setState({ week3RoundEdit }) 
        return
      } else {
        var theYear = item.time.split('-')[0]
        if (Number(theYear) - yearNow > 5 || Number(theYear) < yearNow) {
          week3RoundEdit[index]['error'] = 'Year field badly formatted'
          this.setState({ week3RoundEdit })
          return
        } else {
          k++
          ...
          week3RoundEdit[index]['commenceTime'] = theTime
          week3RoundEdit[index]['timeInMillis'] = timeInMillis
          week3RoundEdit[index]['error'] = ''
      ...
      if (k === 2) {
        ...
        this.state.superBowlEdit.map((item, index) => {
          ...
          if (l === 1) {
            this.setState({ superBowlEdit })
            this.getOddsApiData(week3RoundEdit)
            //console.log('week3RoundEdit', this.state.week3RoundEdit)
  ...
  cancelEdit=()=>{
    week1RoundEdit=week1RoundEdit2,week2RoundEdit=week2RoundEdit2,week3RoundEdit=week3RoundEdit2,superBowlEdit=superBowlEdit2
    this.setState({week1RoundEdit:[],week2RoundEdit:[],week3RoundEdit:[],superBowlEdit:[]},()=>{
    ...
    theArr.map((item, index) => {
      ...
      [editTime]:minTime,[lastMatchTime]:maxTime,currentSelection:this.state.currentSelection,oddsTimeUpdate:nowTime,[postTime]:nowTime}
       theArr.map((item,index) => {
         ...
         // //console.log('eventIdsEditjjjjj',eventIdsEdit)
           ...
           generalDb.child(dataLink).update(toDbArr,(error) => {
            if (error) {
              ...
              this.setState({ showProgressBar: false })
             // var oddsServerLink='theEvents::NFLRegular::'+eventKey+'::'+this.state.currentSelection+'::'+editTime
