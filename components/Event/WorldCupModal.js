...
import CountryCodes from '../TheJSONS/countryCodes.json'
...
var round1Edit = [], round2Edit = [], roundOf16Edit = [], quarterFinalsEdit = [], semiFinalsEdit = [], finalRoundEdit = []
...
class NCAAModal extends Component {
  state = {
    round1Edit: round1Edit, round2Edit: round2Edit3, conferenceChampionshipEdit: conferenceChampionshipEdit3, superBowlEdit: superBowlEdit3, submitErr: "", showProgressBar: false, currentSelection: this.props.eventToWorldCupModal, isItSubmit: false,
    eventStartTime: 0, eventEndTime: '', theNewArr: [], loadApiData: false, getFromOddsApi: false, firstTime: '', sweet16: [], roundOf16Edit: [], quarterFinalsEdit: [], semiFinalsEdit: [], finalRoundEdit: []
  }

  componentDidMount = () => {
    // var incomingData = JSON.parse(JSON.stringify(this.props.itemsToWorldCupModal));
    var incomingData = this.props.itemsToWorldCupModal.slice(0, 2).map(item => JSON.parse(JSON.stringify(item)));
    ...
    if (incomingData.length > 0) {
      ...
      round1Edit = [], round2Edit = [], roundOf16Edit = [], quarterFinalsEdit = [], semiFinalsEdit = [], finalRoundEdit = []
      incomingData.map((item, index) => {
        ...
        if (!item.player1NickName) {
          ...
          if (this.state.currentSelection === 'roundOf32') {
      ...
      //console.log('modal roundOf16Edit',this.state.currentSelection)
      if (this.state.currentSelection === 'round1') { round1Edit = incomingData, this.setState({ round1Edit }) }
      if (this.state.currentSelection === 'roundOf32') { round2Edit = incomingData, this.setState({ round2Edit }) }
      if (this.state.currentSelection === 'roundOf16') { roundOf16Edit = incomingData, this.setState({ roundOf16Edit }) }
  ...
  inputChange = async (e, index, type) => {
    ...
    if (type === 'roundOf16') {
      roundOf16Edit[index][e.target.id] = value
      await this.setState({ roundOf16Edit })
      //console.log("roundOf16Edit", roundOf16Edit)
    }
    if (type === 'quarterFinals') {
  ...
  round2Submit = async (theArr) => {
    ...
    var teamsArr = [],k=0
     theArr.map((item, index) => {
          ...
          if (theArr.length === k) {
            console.log('this.state.round2Edit')
           this.sortOddsJson(theArr, 'round2Edit')
          }
        })
  ...
  sweet16Submit = async () => {
    ...
    /* this.state.roundOf16Edit.map((item, index) => {
       ...
       roundOf16Edit[index]['team2Id'] =this.getRandom(1, 90);
 
       roundOf16Edit[index]['team1Seed'] =this.getRandom(1, 16);
       roundOf16Edit[index]['team2Seed'] =this.getRandom(1, 16);
 
       roundOf16Edit[index]['apiId'] =this.getRandom(1, 100);
      /* if(index===15){
         roundOf16Edit[index]['apiId'] =''
       }
       //roundOf16Edit[index]['time'] =time
       if(this.state.roundOf16Edit.length===index+1){
         //console.log('roundOf16Edit iiiiii',roundOf16Edit)
       }
     })*/
    this.state.roundOf16Edit.map((item, index) => {
      if (item.team1Id === '' || item.team2Id === '' || item.team1Seed === '' || item.team2Seed === '') {
        roundOf16Edit[index]['error'] = 'Teams id, seed & match time field must be filled'
        ...
        if ((this.state.getFromOddsApi && item.apiId === '') || !this.state.getFromOddsApi && item.time === '') {
          ...
          var timeInMillis = new Date(matchTime).getTime()
          var theTime = dayjs(timeInMillis).format('MMM D, YYYY h:mm A')
          roundOf16Edit[index]['commenceTime'] = theTime
          roundOf16Edit[index]['timeInMillis'] = timeInMillis
          roundOf16Edit[index]['error'] = ''
          if (this.state.getFromOddsApi === true) { roundOf16Edit[index]['time'] = theTime }

          thePoints.map((item2) => {
            if (item.team1Seed === item2.seed) { roundOf16Edit[index]['p1Points'] = item2.val }
            if (item.team2Seed === item2.seed) { roundOf16Edit[index]['p2Points'] = item2.val }
          })
        }
      }
      if (this.state.roundOf16Edit.length === j) {
        //console.log('roundOf16Edit jjjjjjj',roundOf16Edit)
        ...
        apiFirebaseRef.once('value', dataSnapshot => {
          if (dataSnapshot.exists()) {
            ...
            dataSnapshot.forEach((data, index) => {
              ...
              if (theCount === i) {
                //console.log('final arrr 111111',teamsArr)
                this.goToTeamsDataApi(roundOf16Edit, teamsArr, 'roundOf16Edit')
              }
            })
          } else {
            //console.log('the teams data nooot available')
  ...
  sortOddsJson = async (theArr, stateEdit) => {
    ...
    var oddsApi = "https://api.the-odds-api.com/v4/sports/soccer_fifa_world_cup/odds?regions=us&markets=h2h&oddsFormat=american&apiKey=" + oddsApiKey
    const response = await axios.get(oddsApi)
    var theOddsJson = response.data
    //var theOddsJson=theWorldCupOdds
    var firstMatchTime = []
    try {
            ...
            if (m === theOddsJson.length) {
              ...
              theArr.map((item, index) => {
                newOddsJson.map((item2) => {
                  // console.log('malizaaaaa 000024',item.apiId,item2.apiId)
                  if (item.apiId === item2.apiId) {
                    // firstMatchTime.push(item2.timeInMillis)
                    // console.log('freeeshy',item.apiId,item2.apiId)
                    ...
                    theArr[index]['p1Photo']=this.getFlagUrl(item2.player1)
                    theArr[index]['p2Photo']=this.getFlagUrl(item2.player2)
                  //  console.log('malizaaaaa mnyoge', theArr)
                  }
                })
              ...
              if (theArr.length === index + 1) {
                ...
                var m=0
                 theArr.map((item, index) => {
      if (item.player1 === 'N/A' || item.player2 === 'N/A' || item.p1Photo === 'N/A' || item.p2Photo === 'N/A') {
        theArr[index]['error'] = 'Some field missing data'
        this.notify('Some field missing data')
        return
      } else {
        m++
        theArr[index]['error'] = ''
  ...
  return code ? `https://flagcdn.com/w320/${code}.png` : 'https://flagcdn.com/w320/un.png'; 
};
  cancelEdit = () => {
    round1Edit = [], round2Edit = [], roundOf16Edit = [], quarterFinalsEdit = [], semiFinalsEdit = [], finalRoundEdit = []
    this.setState({ round1Edit: [], round2Edit: [], roundOf16Edit: [], quarterFinalsEdit: [], semiFinalsEdit: [], finalRoundEdit: [] }, () => {
  ...
  sendToFirebaseSingle = async (editTime, theSelection) => {
    ...
    if (this.state.currentSelection === 'roundOf32') { theArr = this.state.round1Edit, editTime = 'stopRound2Edit' }
    var i = 0
    theArr.map((item, index) => {
      ...
      if (i === theArr.length) {
        ...
        var eventIdsLink2 = '/WorldCup/eventsIds/' + eventKey + '/'
        var dataLink = '/WorldCup/' + eventKey + '/' + this.state.currentSelection//1737147600000
  ...
  sendToFirebaseSingle3 = async () => {
    ...
    theArr.map((item, index) => {
      ...
      if (i === theArr.length) {
        ...
        var eventIdsLink = '/eventsIds/' + eventKey + '/'
        var eventIdsLink2 = '/WorldCup/eventsIds/' + eventKey + '/'
        var dataLink = '/WorldCup/' + eventKey + '/final/' + this.state.currentSelection//1737147600000
        //console.log('combined items',theArr.length, theArr)//1737235800000
        var eventIdsEdit = {
          [editTime]: minTime, currentSelection: this.state.currentSelection
        }
  ...
  itemComponent = (compItems, type) => {
    //console.log('compItems',roundOf16Edit,compItems,type)
    return (
          ...
          <div className={styles.listDiv} key={index}>
            <div className={styles.theCont0}>
              <div className={styles.theCont01}>
                ...
                {item.commenceTime?<p>{item.commenceTime}</p>:null}
              ...
              <div className={styles.theCont}>
                <div className={styles.theContLeft}>
                 ...
                 <input className={styles.P1} id='apiId' value={item.apiId} placeholder='Enter odds api id' onChange={(event) => this.inputChange(event, index, type)} /> 
                ...
                <div className={styles.theContRight}>
                  ...
                  {/*type==='round1'||type==='roundOf32'?<input className={styles.P1} id='team2Id' value={item.team2Id}  placeholder='Enter team 2 team id' onChange={(event) => this.inputChange(event, index, type)} />:<input className={styles.P1} id='time' type='datetime-local' value={item.time} placeholder='Enter match time' readOnly />*/}
                  <input className={styles.P2} id='player2' value={item.player2} placeholder='Enter team 2 name' readOnly/>
                 
                  <input className={styles.P2} id='p2Photo' value={item.commenceTime||'N/A'}  readOnly />
  ...
  render() {
    return (
      <><div className={styles.container2} onClick={(event) => this.doNothing(event)}>
        ...
        <p className={styles.headP}>Enter World Cup Match Details</p>
        {this.state.currentSelection === 'roundOf32' ?
          <div className={styles.divCont}>
            <p className={styles.listHeadP}>Round of 32</p>
            <div className={styles.listCont}>{this.itemComponent(round2Edit, 'roundOf32')}</div></div> : null}
        {this.state.currentSelection === 'roundOf16' ?
          <div className={styles.divCont}>
            <p className={styles.listHeadP}>Round of 16</p>
            <div className={styles.listCont}>{this.itemComponent(roundOf16Edit, 'roundOf16')}</div></div> : null}
        {this.state.currentSelection === 'quarterFinals' ?
          <div className={styles.divCont}>
            <p className={styles.listHeadP}>Quarter Finals</p>
            <div className={styles.listCont}>{this.itemComponent(quarterFinalsEdit, 'quarterFinals')}</div></div> : null}
        {this.state.currentSelection === 'semiFinals' ?
          <div className={styles.divCont}>
            <p className={styles.listHeadP}>Semi Finals</p>
            <div className={styles.listCont}>{this.itemComponent(semiFinalsEdit, 'semiFinals')}</div></div> : null}
