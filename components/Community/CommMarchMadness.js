...
class CommMarchMadness extends Component {
  ...
  runCustomLogic = (theEventKey, currentSelection, sportType,endTime,isEventExpired) => {
    ...
    this.timerHandle = setTimeout(
        () =>{
               this.setState({isEventStarted:this.props.eventStarted,theEventKey:theEventKey,flockNameWithNoSpaces:this.props.flockNameWithNoSpaces,flockNameAvailable:this.props.flockNameAvailable,currentRound:'round1',endTime:endTime,menuToShow:'Rams In Your Flock',currentSubSelection:'round1'},()=>{
this.getRamMembersData()
 
})
            {this.state.isAdmin ? this.loadAdminData(theEventKey, '2ndFeed'):null}
        },
        1000)*/
  };
  checkAuth = () => {
    ...
    return (
      <><div>
        {this.state.menuToShow === 'Rams In Your Flock' && this.state.currentRound !== 'overall' ? <>{this.state.flockNameAvailable ? <div className={styles.menu2Div1}>
          ...
          <div id={styles.table1Div}>
            ...
            <table className={styles.table1}>
              <tr id={styles.table1Tr1}>
                ...
                <th>RAM Name</th>
                {this.state.currentRound === 'finalRound' ? <th>Picked?<br /><span className={styles.subSpan}>{this.state.currentSubSelection}</span></th> :
                  <th>Picked?</th>}
                {this.state.currentRound !== 'finalRound' ? <><th>Best Possible<br />Score</th>
              ...
              {theItems.map((item, index) => {
                ...
                return (
                  <tr key={index} id={styles.table1Tr2} style={{ backgroundColor: item.uid === this.state.userId ? '#292f51' : null, color: item.uid === this.state.userId ? 'white' : '#292f51' }}>
                    ...
                    <td>{item.name}</td>
                    <td>{thePick}</td>
                    {this.state.currentRound !== 'finalRound' ? <><td>{item.BPS}</td>
                      <td>{item.score}</td></> :
                      <>
                        <td>{item.score}</td>
          ...
          <p className={styles.noDataP0} style={{ color: this.state.isEventStarted ? statCol : null }}>{statToShow}</p>
        ...
        {this.state.menuToShow === 'Flocks Among Flocks' || this.state.currentRound === 'overall' ? <>{flockNameAvailable ? <div className={styles.menu2Div1}>
        ...
        {this.state.menuToShow === 'Admin' && this.state.isAdmin && this.state.currentRound !== 'overall' ? <>{this.state.theAdminFlocksArr.length > 0 ? <div className={styles.menu2Div1}>
