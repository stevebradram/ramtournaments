...
class Friends extends Component {
  state = {
    ...
    friendsListArr: [], myUserId: '', isLogged: '',friendsListAvailable:false
  }
  ...
  getData = (userId) => {
    ...
    messageRef.once('value', dataSnapshot => {
      ...
      if(dataSnapshot.exists()){this.setState({friendsListAvailable:true})}
    ...
    return (
      <><div className={styles.container2} onClick={(event) => this.doNothing(event)}>
        <div className={styles.headerDiv}>
          <h2 style={{fontSize:16,fontWeight:600}}>My Friends</h2>
        </div>
