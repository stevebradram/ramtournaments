...
import styles from './MessagesGames.module.scss'
...
var flockTeamName = ''
const mainCard = [
  { id: 1, matchNo: 1, time: 'Nov 3 2024, 03:00PM', player1: 'Brandon Moreno-Mexico', p1Rec: '21-8-2', p1Points: '1.42', player2: 'Amir Albazi-Iraq', p2Rec: '17-1-0', p2Points: '6.48', stat: 'player', game: 'UFC', p1Photo: 'player1.png', p2Photo: 'player2.png', status1: 'notPlayed', bestPossibleScore: '', status2: '', bet: 'player2', winner: 'player1', match: 'Mens Flyweight',myPicks:[],eventTitle:'March Madness 2026 - Round of 64',flockName:'Kim Zeus',ramName:'Malick Berry'},
    ...
    return (
      <><div className={styles.container2} onClick={(event) => this.doNothing(event)}>
        <div className={styles.listCont}>
          <p className={styles.eventTitle}>{pickDetails[2]}</p>
          <p className={styles.teamName} style={{ color: theUser === 'me' ? '#fff' : null }}>RAM Name: {pickDetails[0]}</p>
          <p className={styles.flockName} style={{ color: theUser === 'me' ? '#fff' : null }}>Flock Name: {pickDetails[1]}</p>
          {this.props.myPicks.map((item, index) => {
            var theId = item.id
            var theBet = item.bet
            ...
            return (
              <div className={styles.listDiv} key={index} style={{ backgroundColor: theUser === 'me' ? '#292f51' : null, borderColor: theUser === 'me' ? '#444d81ff' : null }}>
                    ...
                    {tableArray.map((item, index) => {
                      ...
                      return (
                        <tr id={styles.table1Tr2} key={index}>
                          ...
                          <td style={{ color: theUser === 'me' ? '#fff' : null }}>{(theItem[0]).slice(0,24)}</td>
                          <td style={{ color: theUser === 'me' ? '#fff' : null }}>+{(theItem[1]).slice(0,24)}</td>
                        </tr>
                      )
                    })}
                  </table> </div>
