...
class TheMarchMadness extends Component {
  ...
  state = {
    week1RoundArr: [], week2RoundArr: [], week3RoundArr: [], currentSelection: 'week1', theItems: [], finalRoundExists: false, isAdmin: false, userId: '', overallArr: [], overallRoundExists: '', finalRoundArr: [], finalRoundMenu: '', theCount: '', deleteModal: false,
    deleteName: '', userIdToBeDeleted: '', flockToBeDeleted: '', thePicked: '', eventStartTime: ''
  }
  ...
  getEventStartTime = async () => {
    var eventStartTimeRef = firebase.database().ref('/theEvents/eventsIds/' + this.props.theEventKey + '/stopweek1RoundEdit')
    eventStartTimeRef.once('value', dataSnapshot => { this.setState({ eventStartTime: dataSnapshot.val() }) })
  ...
  deleteMember = () => {
    ...
    //return
     if(this.state.eventStartTime&&this.state.userIdToBeDeleted&&this.props.theEventKey&&this.props.sportType){
 ...
 this.notify('Some data missing')
  ...
  deleteMember2 = () => {
    ...
    generalDb.child('users/' + this.state.userIdToBeDeleted + '/flockData/flockNames/' + this.props.theEventKey).set(null)
    userRef.child('events/' + this.props.sportType + '/' + this.props.theEventKey).set(null)
    userRef.child('upcomingEvents/' + this.props.sportType + '/' + this.props.theEventKey).set(null)
    userBets.child(this.props.sportType + '/' + this.props.theEventKey + '/' + this.state.userIdToBeDeleted).set(null)
    //userBets.child('/scoreBoards/'+this.props.sportType+'/'+this.props.theEventKey+'/'+this.state.userIdToBeDeleted).set(null)
    //userBets.child('/ranking/'+this.props.sportType+'/'+this.props.theEventKey+'/'+this.state.userIdToBeDeleted).set(null)
        ...
        <div>
          ...
          <div className={styles.menu2Div1}>
            {this.state.isAdmin ? <div id={styles.exportDiv}> <div id={styles.exportDiv1} onClick={() => this.notify('Downloading...')}><DownloadTableExcel
              filename={this.props.theEventKey}
              sheet="users"
