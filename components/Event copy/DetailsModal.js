...
class DetailsModal extends Component {
  state={teamName:'',flockName:'',flockName2:'',teamNameErr:'',flockNameErr:'',upcomingRamUfcDetails:true,
    userId:'',theItems:this.props.theItems,allPicked:true,currentEvent:'',mySelection:[],
  ...
  submitDetails=()=>{
    ...
    this.state.theItems.map((item,index)=>{
      //console.log('it iiiis',item.bet,index)
      var amount=0
      if(item.bet==='player1'){
   ...
   toDatabase=()=>{
    ...
    gamesDataRef.child(this.props.theEventKey+'/details/'+this.props.eventType+'/').update(detailsData)
    gamesDataRef.child(this.props.theEventKey+'/bets/'+this.props.eventType+'/').update(itemsData)
    uniqueRamNamesRef.child(theTeamName).set(this.state.userId)
    uniqueFlockNamesRef.child(theFlockName).set(this.state.userId)
    ramsBets.child(this.props.theEventKey+'/'+this.props.eventType+'/').child(this.state.userId).update(itemsData,(error) => {
      if (error) {
...
//console.log('theItems 1111111111',theItems)
  ...
  render() {
    var theEvent='',eventType=''
    if(this.props.currentEvent==='ramUfc'){theEvent='RAM UFC'}
    if(this.props.eventType==='mainCard'){eventType='MAIN CARD'}
    if(this.props.eventType==='prelims'){eventType='PRELIMS'}
    if(this.props.eventType==='earlyPrelims'){eventType='EARLY PRELIMS'}
    return (
      <><div className={styles.container2} onClick={(event)=>this.doNothing(event)}>
        ...
        {this.state.theItems.map((item,index)=>{
          ...
          return(
           ...
           <p className={styles.matchP}>Match #{item.matchNo}</p>
